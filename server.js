...
/** Create docker proxy */
var createDockerProxy = function(options) {
  assert(options && options.port, "options.port must be given");
  ...
  server.on('upgrade', function(clientReq, clientSocket, clientHead) {
  ...
  return new Promise(function(accept, reject) {
    server.once('listening', function() {
      debug("Server listening on port: %s", options.port)
      accept(server);
    });
...
if (!module.parent) {
  var port = parseInt(process.env.PORT || 2375);
  createDockerProxy({
    port:         port
  }).then(function() {
    console.log("dind-service-proxy is listening on port: %s", port);
  }).catch(function(err) {
